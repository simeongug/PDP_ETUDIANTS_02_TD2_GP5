...
const char *wifi_ssid = "TZ";
const char *wifi_password = "LaTZPartout";
...
const char *client_id = "TD02_GP05";
////
...
void setup()
{
  ...
  if (mqtt_client.connect(client_id, mqtt_user, mqtt_pass))
  {
    ...
    mqtt_client.publish ("TD02_GP05/temp", String(event.temperature).c_str());
    mqtt_client.publish ("TD02_GP05/relhum", String(event.relative_humidity).c_str());
  }
